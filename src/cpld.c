...
extern Uint16 CompensationCnt;
extern Uint16 MonitorBuffer1Cnt, MonitorBuffer2Cnt, MonitorBuffer3Cnt,MonitorBuffer4Cnt;
Uint32 AverageCounter;

Uint32 	DA_CONVERTCnt = 0;
Uint32	DA_MirrorCycleCnt = 0;

extern Uint32 	DA_CONVERTCnt;
extern Uint32	DA_MirrorCycleCnt;
...
AD5315DA  Ad_Convert_2_Sync;
AD5315DA  Ad_Convert_3;
extern DACONVERT  DA_AD5315_G106;//DA_G106
...
volatile INT16U * READ_I2C_1  	= (INT16U *) mCPLDADDR_READ_I2C_1; 	//R  data=bit0~7
volatile INT16U * READ_I2C_2  	= (INT16U *) mCPLDADDR_READ_I2C_2; 	//R  data=bit0~7
volatile INT16U * READ_I2C_3  	= (INT16U *) mCPLDADDR_READ_I2C_3; 	//R  data=bit0~7
volatile INT16U * READ_I2C_4  	= (INT16U *) mCPLDADDR_READ_I2C_4; 	//R  data=bit0~7

volatile INT16U * READ_COM_ReadSign  	= (INT16U *) mCPLDADDR_READ_COM_ReadSign ;
volatile INT16U * READ_COM_ReadSign2  	= (INT16U *) mCPLDADDR_READ_COM_ReadSign2 ;
...
volatile INT16U * DA_Convert_Ctrl_2_Sync  	= (INT16U *) mCPLDADDR_DA2_Convert_Sync; //0x994	
volatile INT16U * DA_Convert_CS_2  	= (INT16U *) mCPLDADDR_DA2_CS; //0x998	
...
union SERVO_COM_READSIGN_REG	Servo_Com_ReadSign_Register2;
...
{		
	...
	{	
		...
		{
			Servo_Coin1_In_Register.bit.COIN_15=~Servo_Coin1_In_Register.bit.COIN_15;
			Servo_Coin1_In_Register.bit.COIN_16=~Servo_Coin1_In_Register.bit.COIN_16;
			Servo_Coin1_In_Register.bit.COIN_17=~Servo_Coin1_In_Register.bit.COIN_17;
			Servo_Coin1_In_Register.bit.COIN_18=~Servo_Coin1_In_Register.bit.COIN_18;
		}
		//Servo_Coin1_In_Register.bit.COIN_11=Inposition_Signal_Valid;
		//Servo_Coin1_In_Register.bit.COIN_12=Inposition_Signal_Valid;
		Servo_Coin1_In_Register.bit.COIN_13=Inposition_Signal_Valid;
...
void Read_Servo_Coin2_In(void)
{
	...
	{	
		...
		if(Servo_Coin2_In_Register.bit.COIN_21 == Inposition_Signal_Invalid)
		{
			Servo_Coin2_In_Register.bit.COIN_21 = Servo_Coin2_In_Register.bit.COIN_21;
		}
		...
		{	
			Servo_Coin2_In_Register.bit.COIN_19=Inposition_Signal_Valid;//for test Beta ===>L18_Slave Coin.
			Servo_Coin2_In_Register.bit.COIN_20=~Servo_Coin2_In_Register.bit.COIN_20;//for test
			Servo_Coin2_In_Register.bit.COIN_21=~Servo_Coin2_In_Register.bit.COIN_21;
			Servo_Coin2_In_Register.bit.COIN_22=~Servo_Coin2_In_Register.bit.COIN_22;
		}
...
void Read_Serial_Com_ReadSign2(void)
{
	Servo_Com_ReadSign_Register2.all = *READ_COM_ReadSign2;
}
...
void Write_DA_CONVERT_CONTROL_2(DACONVERT  *DA_WavePara)
{
	...
	{
		if((DA_WavePara->AutoSign == 0x02)||(DA_WavePara->AutoSign == 0x03))
		{
		...
		else if((DA_WavePara->AutoSign == 0x04)||(DA_WavePara->AutoSign == 0x05))
		{
			*AOTF_HighCount = 0;
			*AOTF_LowCount = 0;
			*AOTF_Phase = 0;
	...
	*DA_Convert_Ctrl_2 = Ad_Convert_2.OutData;
	...
	Ad_Convert_2_Sync.OutData = 0;
	Ad_Convert_2_Sync.AD5315Str.Data = DA_WavePara->DA_Data_Sync;
	
	Ad_Convert_2_Sync.AD5315Str.Addr1 = (DA_WavePara->AutoSign == 0x02)?(1):(0);
	Ad_Convert_2_Sync.AD5315Str.Addr2 = (DA_WavePara->AutoSign == 0x03)?(1):(0);
	Ad_Convert_2_Sync.AD5315Str.Addr3 = (DA_WavePara->AutoSign == 0x04)?(1):(0);
	Ad_Convert_2_Sync.AD5315Str.Addr4 = (DA_WavePara->AutoSign == 0x05)?(1):(0);
	
	*DA_Convert_Ctrl_2_Sync = Ad_Convert_2_Sync.OutData;
...
void Write_DA_CONVERT_CONTROL_PC(void)
{
	static Uint8 PC_Cal_Modle_Sel = 1;		//0:triangle;1:sin;2:cos;3:atan;4:custom
	//PC_Cal_Modle_Sel = DA_AD5315_G106.AutoSign;
	PC_Cal_Modle_Sel = DA_AD5315_G106.WaveSelect;
	#if 1		
	OverallSign.Int4IdleSign_4 = 0;
	...
	{
		Write_DA_CONVERT_CONTROL_PC_CAL(PC_Cal_Modle_Sel);
	}
...
void Write_DA_CONVERT_CONTROL_PC_CAL(Uint8 Cal_Modle_Sel)
{
	DACONVERT DA_ConvertData;
		
	static int DA_ConvertEndValue = 950;
	DA_ConvertEndValue = DA_AD5315_G106.Voltage_Max;
	...
	float64	Rad_angle,K_slope;
	int 		amplitude ;

	static int DA_ConvertEndValue_Sync = 800;
	DA_ConvertEndValue_Sync = DA_AD5315_G106.Voltage_Max2;
	static int DA_ConvertSatrtValue_Sync = 200;
	DA_ConvertSatrtValue_Sync = DA_AD5315_G106.Voltage_Start2;
	int 		DA_ConvertNowValue_Sync;	
	float64	Rad_angle_Sync,K_slope_Sync;
	int 		amplitude_Sync ;	

	static Uint8 	DA_CalculateSign = 0;
	static Uint32 	DA_ConvertMonitorCnt = 500;
	static Uint8 	AutoSign_Test = 0;
	static Uint8	DA_ADDR = 3;

	if(DA_CalculateSign == 0)
	{
		DA_CalculateSign = 1;
	}

	amplitude = divdp((DA_ConvertEndValue - DA_ConvertSatrtValue),2);
	amplitude_Sync =  divdp((DA_ConvertEndValue_Sync - DA_ConvertSatrtValue_Sync),2);

	if(DA_MirrorCycleCnt >= DA_MirrorCycleTotal)
	{
		DA_MirrorCycleCnt = 0;
		DA_CONVERTCnt = 0;
		DA_CalculateSign = 0;
	...
	else
	{
		...
		if(DA_CONVERTCnt >= DA_ConvertCntTotal)
		{
			DA_MirrorCycleCnt++;
			DA_CONVERTCnt = 0;		
		}
		...
		if(Cal_Modle_Sel == 0)
		{
			...
			if(DA_CONVERTCnt < divdp(DA_ConvertCntTotal,2))
			{
				...
				DA_ConvertNowValue = K_slope * DA_CONVERTCnt + DA_ConvertSatrtValue;

				K_slope_Sync = divdp(amplitude_Sync*4,DA_ConvertCntTotal);
				DA_ConvertNowValue_Sync = K_slope_Sync * DA_CONVERTCnt + DA_ConvertSatrtValue_Sync;
				
			}
			else
			{
				K_slope = -divdp(amplitude*4,DA_ConvertCntTotal);		
				DA_ConvertNowValue = amplitude*2 + K_slope * (DA_CONVERTCnt - divdp(DA_ConvertCntTotal,2)) + DA_ConvertSatrtValue;

				K_slope_Sync = -divdp(amplitude_Sync*4,DA_ConvertCntTotal);		
				DA_ConvertNowValue_Sync = amplitude_Sync*2 + K_slope_Sync * (DA_CONVERTCnt - divdp(DA_ConvertCntTotal,2)) + DA_ConvertSatrtValue_Sync;
			}
		}
		else if(Cal_Modle_Sel == 1)
		{
			...
			DA_ConvertNowValue_Sync= sindp(Rad_angle) * amplitude_Sync + (DA_ConvertSatrtValue_Sync + amplitude_Sync);
		}
		else if(Cal_Modle_Sel == 2)
		{
			...
			DA_ConvertNowValue_Sync = cosdp(Rad_angle) * amplitude_Sync + (DA_ConvertSatrtValue_Sync + amplitude_Sync);
		}
		...
		//Write_DA_CONVERT_CONTROL(DA_ConvertNowValue,0x04,0);

		DA_ConvertData.DA_Data = DA_ConvertNowValue;
		DA_ConvertData.DA_Data_Sync = DA_ConvertNowValue_Sync;
		...
		||((DA_CONVERTCnt > (DA_ConvertCntTotal/2 + DA_AD5315_G106.StartOffsetCnt))&&
			(DA_CONVERTCnt <= (DA_ConvertCntTotal-DA_AD5315_G106.EndOffsetCnt))))
		{
			DA_ConvertData.AutoSign = DA_AD5315_G106.AutoSign;
		}
		else
		{
			DA_ConvertData.AutoSign = 0;
		}

		DA_ConvertData.DA_CS = 0;
		Write_DA_CONVERT_CONTROL_2(&DA_ConvertData);
	}
...
{
	//HwiPulse1_Func		 //GPIOINT0A	GPIO0_0  //GPIO0_0 int64   reserved  gpio0 pin=gpio0_bit24;DSP	INT 64
	GPIO_IRQStatus.GPIO0_IRQStatus_RAW_0 = GPIO0_IRQSTATUS_RAW_0_REG;
	//*(GPIO_IRQStatus.GPIO0_IRQStatus_RAW_0) = 0;

	//LPMEN2	HwiPulse2_Func	//GPIO0_1 int65 pulse_interpolation  gpio0_1 pin=gpio0_1 bit23; DSP INT 65
	GPIO_IRQStatus.GPIO0_IRQStatus_RAW_1 = GPIO0_IRQSTATUS_RAW_1_REG;
	//*(GPIO_IRQStatus.GPIO0_IRQStatus_RAW_1) = 0;

	//LPMEN1	HwiPulse3_Func		//GPIO1_0	//int66 pulse_interpolation gpio1_0 pin=gpio1_0_bit17;DSP  INT 66
	GPIO_IRQStatus.GPIO1_IRQStatus_RAW_0 = GPIO1_IRQSTATUS_RAW_0_REG;
	//*(GPIO_IRQStatus.GPIO1_IRQStatus_RAW_0) = 0;

	//HwiPulse4_Func		//GPIO1_1	int67 reserved gpio1_1 pin=gpio1_1_bit19;DSP  INT 67
