...
/************************************************************************************	
函数名称：u8 Value_Char(u8 Value)
函数功能：数值转换成字符		   
入口参数：Char:转换的数值
返回参数：转换后的字符
************************************************************************************/  	
Uint8 Value_Char(Uint8 Value)
{
	switch(Value)
	{
		case 0:	return '0';
...
{	//LED 电压控制
	...
	* COM_IN_FIFO_Clear = 0xffff;
	...
	DA_AD5315_G106.Voltage_Max2 = GCodeRIPointer->StartPoint.Axis15;
	...
	if(DA_AD5315_G106.AutoSign != False)
	{
		...
		if(GCodeRIPointer->StartPoint.Axis5 == 5)
		{
			DA_AD5315_G106.DA_ADDR_Sync = 0x01;
		}
		else if(GCodeRIPointer->StartPoint.Axis5 == 6)
		{
			DA_AD5315_G106.DA_ADDR_Sync = 0x02;
		}
		else if(GCodeRIPointer->StartPoint.Axis5 == 7)
		{
			DA_AD5315_G106.DA_ADDR_Sync = 0x04;
		}
		else if(GCodeRIPointer->StartPoint.Axis5 == 8)
		{
			DA_AD5315_G106.DA_ADDR_Sync = 0x08;
		}
		else
		{
			DA_AD5315_G106.DA_ADDR_Sync = 0;
		}
		...
		if(DA_AD5315_G106.AutoSign > 1)
		{	//AutoSign=2,3,4,5
			DA_AD5315_G106.StartOffsetCnt = GCodeRIPointer->StartPoint.Axis13*2;
			DA_AD5315_G106.EndOffsetCnt = GCodeRIPointer->StartPoint.Axis14*2;
		}
		else
		{
...
{	//移液泵移液、放液
	...
	/*
	绝对位置以微升表示：
	将活塞移动到1000uL 的绝对位置（0.000-1100.000）
	/1A1000,1R
	
	将活塞移动到位置0（分配全部内含液体）。
	/1A0,1R

	最高速度以微升表示：将活塞速度设置为（2.500-1000.000）
	将活塞速度设置为75 μL /秒（较慢的抽吸速度通常使抽吸的液体有更好的一致性）
	/1V75,1R
	
	将活塞速度设置为625μL/秒（较快的速度有助于更好地排空液体）。
	/1V625,1R
	*/
	...
	{	//need set value
		//	"/1V75,1R"
		//Feedrate_ul = GCodeRIPointer->Feedrate_?;
		*WRITE_RS485_1 = "/";
		*WRITE_RS485_1 = "1";
		*WRITE_RS485_1 = "V";
		*WRITE_RS485_1 = "1";
		...
		*WRITE_RS485_1 = 0x0A;	
		...
		// "/1A0,1R"
		//Abs_ul = GCodeRIPointer->StartPoint.Axis?;
		*WRITE_RS485_1 = "/";
		*WRITE_RS485_1 = "1";
		*WRITE_RS485_1 = "A";
...
Uint16 NC_Run_Gcode(void) 
{
			...
			|| (NCRunSign.GetPositionSign == 1))	// 需要获得稳定的坐标值
	{
		if (OverallSign.EveryAxisCoinValid) 
		{
			if (get_delta_time(Timer0.GetPositionCounter) > 500)		//500ms
			{
